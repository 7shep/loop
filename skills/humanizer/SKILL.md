...
description: |
  ...
  language, stock AI words, bold labels, or filler. In Loop, use it as the required
  final prose pass for a section writer. Based on Wikipedia's "Signs of AI writing."
license: MIT
metadata:
  version: "3.0.0"
---
...
When invoked by Loop's writer, this skill is a required final pass, not a new
agent or state-machine stage. The writer has already drafted the complete
section from the approved plan and registered evidence. Humanize that complete
draft before the writer writes the declared `draft.md` artifact.

Loop-specific rules:

- Treat the approved plan, assignment requirements, reviewer feedback, and
  registered evidence as constraints. The draft is the text to edit, not a new
  research prompt.
- If Loop supplies an approved neighboring section as a voice sample, use it
  only to match tone, sentence shape, and punctuation. Never copy its content
  or citations into the current section.
- Preserve every supported claim, qualification, heading, required format, and
  internal citation marker such as `[S01]`. Keep a marker attached to the claim
  it supports if sentence structure changes. Preserve the full set of markers.
- Leave code blocks, inline code, commands, paths, URLs, and link targets
  unchanged.
- Do not add facts, examples, analysis, sources, citations, links, or a more
  confident interpretation. Do not use the humanizer pass to repair an evidence
  gap; leave the claim alone or flag the problem to the writer.
- Return only the final humanized section text. Do not return the tell list,
  editing notes, a summary, or chat wrapper text. The writer owns the only write
  to the declared draft path.
- If a requested change would drop a claim or citation marker, keep the safer
  wording and tell the writer what needs attention rather than guessing.

After the pass, the writer must verify that the output is non-empty and that
the citation-marker set and supported claims still match the pre-humanization
draft. The normal Loop writing reviewer runs afterward; humanizer is a prose
quality pass, not an evidence or rubric review.
